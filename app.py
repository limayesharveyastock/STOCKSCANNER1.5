...
def run_integrated_pipeline():
    ...
    for index, symbol in enumerate(nifty_200_symbols):
        ...
        try:
            ...
            scan_results.append({
                ...
                # Multi-Timeframe Matrix Output Mapping
            ...
            time.sleep(0.25) # Throttle padding for extended index loop
    ...
    if not scan_results:
        st.warning("No tracking records generated.")
        return
    ...
    # --- TAB 1: TECH SCANNER WORKSPACE ---
    with tab1:
        st.subheader("⚙️ Timeframe Filter Setup")
        active_tf = st.radio("Select Active Scanner Timeframe Layer:", ["15 Minute", "1 Day"], horizontal=True)
        ...
        c1.metric("Total Nifty 200 Assets", len(master_df))
        c2.metric(f"Bullish Volume Breakouts", len(bullish_df))
        c3.metric(f"Bearish Volume Breakdowns", len(bearish_df))
        c4.metric(f"Neutral/Consolidating Grid", len(neutral_df))
        ...
        st.subheader(f"🔥 Vol Surge Buy Configurations ({active_tf})")
        if not bullish_df.empty:
            st.dataframe(bullish_df[tech_display_cols], use_container_width=True, hide_index=True)
        else:
            st.info("No Nifty 200 setups matching breakouts on this timeframe.")
            
        st.divider()
        
        st.subheader(f"❄️ Vol Surge Sell Configurations ({active_tf})")
        ...
        st.subheader(f"⚖️ Watchlist Baseline Matrix ({active_tf})")
        if not neutral_df.empty:
            st.dataframe(neutral_df[tech_display_cols], use_container_width=True, hide_index=True)

    # --- TAB 2: MACRO STRUCTURAL GRID (BIFURCATION) ---
    with tab2:
        ...
        with f_col1:
            ...
            selected_industry = st.selectbox("Select Target Sector:", all_industries)
        ...
        with f_col2:
            ...
            selected_tier = st.selectbox("Select Insider Stake Strength:", all_tiers)
        ...
        if not bifurcated_df.empty:
            ...
            st.warning("No asset structural profiles match the selected configuration parameters.")
            
    st.write(f"Pipeline Sync Complete | Timestamp: {datetime.now().strftime('%H:%M:%S')}")
